...
function run() {
    ...
    this.daily = function () {
        ...
        var sqlQuery = "SELECT DISTINCT(D.userUserId) AS Users, UDM.fcmToken AS Token FROM dailyFoodLogs AS D INNER JOIN users U ON U.userId = D.userUserId LEFT OUTER JOIN userdevicemappers UDM ON UDM.userUserId = U.userId WHERE DATE(D.dateTimeLogged) < '" + currentDate + "' AND U.logNotifications = 1";
    ...
    this.weekly = function () {
        ...
        var sqlQuery = "SELECT MAX(W.weekId) AS LastWeek, W.userUserId AS UserId, MAX(W.dateAdded) AS LastLoggedDate, UDM.fcmToken from weeklylogs W INNER JOIN users U ON U.userId = W.userUserId LEFT OUTER JOIN userdevicemappers UDM ON UDM.userUserId = U.userId WHERE U.logNotifications = 1 AND DATE(W.dateAdded) < '" + past + "' GROUP BY W.userUserId";
        var dateTimeSent = new Date().toISOString().slice(0, 19).replace('T', ' ');
