...
router.get('/getAppointmentDetails', userAuthenticate, function (req, res) {
    var userId = res.locals.userId || req.session.userId;
    var sqlQuery = "SELECT appointments.* FROM appointments LEFT OUTER JOIN notes on notes.appointmentAppointmentId = appointments.appointmentId WHERE appointments.userUserId = '" + userId + "'";
